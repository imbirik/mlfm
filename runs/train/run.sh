#!/usr/bin/env bash
# Plain bash launcher for the final pretraining/adaptation run.

set -euo pipefail

REPO_ROOT="${REPO_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
CONFIG_PATH="${CONFIG_PATH:-$REPO_ROOT/runs/train/config.yml}"
...
DATA_ROOT="${DATA_ROOT:-$PROJECTDIR/datasets/slimpajama_reupload_125b_1024_tinyllama}"
SFT_DATA_ROOT="${SFT_DATA_ROOT:-$PROJECTDIR/datasets/sft}"
SMDM_CODE_DIR="${SMDM_CODE_DIR:-$REPO_ROOT/external/SMDM}"
DEFAULT_OUTPUT_DIR="$PROJECTDIR/runs/train/output_smdm_{smdm_size}M_pretrain"
...
CMD=(
  ...
  --config_override "data_paths=[$DATA_ROOT/train]"
  --config_override "eval_data_paths=[$DATA_ROOT/validation]"
  --config_override "data_mix_weights=[1.0]"
  ...
  --config_override "sft_code_data_paths=[$SFT_DATA_ROOT/opencodeinstruct/train]"
